...
CREATE SCHEMA IF NOT EXISTS `bibliotheek` DEFAULT CHARACTER SET utf8;
USE `bibliotheek` ;
...
CREATE TABLE IF NOT EXISTS `loan` (
  ...
  PRIMARY KEY (`LoanDate`,`MemberID`,`CopyID`),
  KEY `MemberID` (`MemberID`),
  KEY `LoanID` (`LoanID`),
  KEY `loan_copy` (`CopyID`)
) ENGINE=InnoDB AUTO_INCREMENT=14748 DEFAULT CHARSET=latin1;
...
CREATE UNIQUE INDEX member_email_index ON `member` (EmailAddress);
...
CREATE UNIQUE INDEX member_city_index ON `member` (City);
DROP INDEX `member_city_index` ON `member`;
